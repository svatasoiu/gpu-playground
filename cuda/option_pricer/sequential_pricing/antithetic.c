...
// antithetic sampling is done by using Z~N(0,1), and -Z.
int mc_antithetic(monte_args_t *args) {
	...
	for (int i = 0; i < args->n_trials; i+=2) {
		...
		double log_S1_ant = log_price;
		double log_S2_ant = log_price;
		...
		args->antithetic[i]   = exp(log_S1_ant);
		args->antithetic[i+1] = exp(log_S2_ant);
	}
	return 0;
}