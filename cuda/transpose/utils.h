...
template <class T>
struct Tranposer {
  float (*transpose)(const T *, T *, const size_t);
  std::string name;
};

#define INIT_TRANSPOSE_FUNC(f, T) template float f<T>(const T *, T *, const size_t)
#define DECL_TRANSPOSE_FUNC(f) template <class T> float f(const T * const, T * const, const size_t)
#define INIT_TRANSPOSER(f, name) { f, name }
...
template<class T>
bool check_matrices(const T * const m1, const T * const m2, const size_t N) {
  for (size_t i = 0; i < N; ++i)
  	for (size_t j = 0; j < N; ++j)
      if (m2[i + j * N] != m1[j + i * N])
        return false;
  return true;
}
